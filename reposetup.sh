...
# # Ensure local environment is linked (helps IDEs and prevents some mount issues)
...
# # ------------------------------------------------------------------
# # FIX APPLIED: Added anonymous volumes for node_modules in services to prevent 
# # host volume mount from hiding container dependencies.
...
#       # but if using venv inside app, this would be needed. 
#       # kept clean for now as we used system pip in container.
...
#   synth-gen:
#     command: python /app/apps/synth-gen/main.py
# EOF
...
# FROM dev AS prod
# CMD ["python", "apps/synth-gen/main.py"]
# EOF
